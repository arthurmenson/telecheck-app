...
1. Add `forms_template_l3_edit_log` table tracking every INSERT or UPDATE to `eligibility_logic` JSONB. Trigger-driven change-data-capture covers BOTH events: entry per change: `(edit_log_id PK, template_id, tenant_id, draft_revision_id, editor_account_id, edited_at, prior_value_hash, new_value_hash, changed_path_set JSONB, value_fingerprint_map JSONB, edit_type ENUM('baseline_insert', 'incremental_update', 'corrective_supersession'))`. **Baseline provenance:** on `forms_template` INSERT (or on any import/migration that produces a non-empty `eligibility_logic`), a synthetic edit-log row of `edit_type='baseline_insert'` is created with `prior_value_hash = baseline_empty_canonical_hash` (SHA-256 of the empty-object JSON `{}`) and `changed_path_set` + `value_fingerprint_map` covering every leaf in the initial eligibility_logic. This row is subject to the same dual-control approval requirement as any incremental edit — first publish is blocked until a tenant_clinical_lead pairs an approval to the baseline-insert row. **`changed_path_set` is the canonical sorted JSONPath array of every leaf that differs between prior_value and new_value; `value_fingerprint_map` is the per-path SHA-256 fingerprint of the new value at each leaf.** Hashes prove tamper-evidence; path-set + fingerprints prove *what* changed and form the artifact a second approver actually reviews. `edit_log_id` is the immutable handle the approval artifact binds to.
   **Append-only enforcement (parity with I-003 audit_records pattern):**
   ...
   - The columns `(changed_path_set, value_fingerprint_map, prior_value_hash, new_value_hash, editor_account_id, edited_at, tenant_id, template_id, draft_revision_id)` are all immutable by construction once the append-only trigger is in place; no migration, repair script, or admin path can rewrite them in-place.

   **Supersession model (explicit; used when a malformed edit-log row needs retirement):**
   - Retirement is a NEW append with `(supersedes_edit_log_id FK NOT NULL → forms_template_l3_edit_log.edit_log_id, supersession_rationale TEXT NOT NULL, superseding_editor_account_id, superseded_at)`. The retiring row is itself a fresh edit-log entry: it carries its own `changed_path_set` + `value_fingerprint_map` representing the corrective diff, and is itself subject to the L3 approval requirement (a corrective edit needs its own paired `forms_template_l3_approval` row).
   - Supersession is INSERT-only: a single `edit_log_id` may be superseded by at most one row (UNIQUE index on `supersedes_edit_log_id WHERE supersedes_edit_log_id IS NOT NULL`); chained supersession (A → B → C) is permitted and is computed transitively by `is_active_edit_log(edit_log_id)` helper.
   - **Pre-existing approvals on a superseded row are invalidated automatically at publish-time** by the active-row predicate below; the approval row itself is NOT deleted (append-only on `forms_template_l3_approval` also), but it ceases to satisfy publish because its parent edit-log is no longer active.
   - **Already-published-template safety:** if a published template's referenced edit-log entry is later superseded, the published template's existing publish provenance is unaffected (publish is point-in-time; the template doesn't re-validate). However, any subsequent re-publish (e.g., new draft revision based on the published version) re-evaluates the L3 gate against the current draft's active edit-log entries; superseded rows from earlier history are not considered in that evaluation. The earlier publish remains historically valid; the corrective edit feeds the next publish cycle.
   - **Audit:** supersession emits `forms.l3_edit_log.superseded` Category B audit with `(superseded_edit_log_id, superseding_edit_log_id, rationale, actor_account_id)`. The audit row is the durable record of why a correction happened.
...
3. Publish path requires **one matching approval row per ACTIVE edit-log entry**, with **publish-time revalidation against the current edit-log row** (defense-in-depth on top of INSERT-time CHECKs):
   - **Active-row predicate:** the publish predicate operates ONLY on `forms_template_l3_edit_log` rows where `is_active_edit_log(edit_log_id)` returns TRUE — i.e., rows not transitively superseded by any later edit-log row on the same draft revision. Superseded rows (and their paired approvals) are categorically excluded; a corrective edit-log row appended after retirement is what requires approval to satisfy publish.
   - For each ACTIVE `forms_template_l3_edit_log` entry on this template's current draft revision, look up `forms_template_l3_approval WHERE edit_log_id = $edit_log.edit_log_id`.
...
4. **State-validating gate (every live leaf must trace to an approved active row):**
   - Compute the current draft's `eligibility_logic` leaf-fingerprint map: for every leaf JSONPath in the live JSON, compute SHA-256 of its canonical-serialized value. This yields `live_leaf_fingerprint_map`.
   - For each leaf in `live_leaf_fingerprint_map`: find the most-recent ACTIVE edit-log row whose `value_fingerprint_map` includes that JSONPath with a matching fingerprint. If no such row exists OR the most-recent active row's fingerprint at that path disagrees with the live leaf → reject with `forms.publish.l3_live_state_unprovenanced` Category B audit including the orphan leaf path + observed live fingerprint.
   - **Supersession coverage CHECK (INSERT-time on the corrective edit-log row):** when a row is inserted with `supersedes_edit_log_id IS NOT NULL`, require that its `changed_path_set` **covers every path** in the superseded row's `changed_path_set` (set inclusion). Each covered path's fingerprint in the corrective row represents the replacement value the approver authorized; paths the corrective row does NOT cover would leave the superseded row's value live without a fresh approval — categorically forbidden by this CHECK. This forces correctives to explicitly reaffirm or replace every path the original edit touched.
   - **Effect:** the gate now validates not just "every edit has an approval" but "every leaf of live eligibility_logic is provenanced by an active approved edit." Cannot bypass via narrow supersession that excludes paths still live in the draft; cannot bypass via fingerprint drift between approval artifacts and live state.
5. Validate the publishing actor's role is in the dual-control authorized set: `clinician` with the `tenant_clinical_lead` tag (TBD where this tag lives — `accounts.tags JSONB` per a future RBAC v1.1 extension, OR a separate `tenant_clinical_lead_assignments` table per tenant).
6. **Explicit batch-approval model (deferred / out of scope for v1.0):** if a future workflow needs to approve N independent edits in one reviewer action, the design is a separate `forms_template_l3_batch_approval` artifact that REFERENCES the set of `edit_log_id`s it covers; each referenced edit must still satisfy all constraints above, and the batch-approval is just a UI/UX convenience around inserting N approval rows atomically. The publish-path predicate still iterates per-edit-log-id — batch approval does not weaken the per-edit binding.
7. **Import / migration / data-fixture pathways:** any code path that creates a `forms_template` row outside the normal authoring UI (admin import, migration-time seed, fixture loader, cross-tenant clone) MUST emit a synthetic `edit_type='baseline_insert'` row covering the entire initial eligibility_logic. There is no provenance-bypass path; templates without a baseline-insert row in their edit-log history fail the live-leaf state-validating gate. Fixture-loaders in tests use the `FORMS_PUBLISH_GATES_TEST_OVERRIDE_L3_DUAL_CONTROL` env var (production-guarded) OR construct paired baseline-approval rows in the same setup; production has no override path.
...
3. Publish path runs the analyzer producing a set of findings `F = {f₁, f₂, ...}` where each `fᵢ = (category, jsonpath, finding_fingerprint)`. Look up the set of bound exemptions `E = {e₁, e₂, ...}` from `forms_template_i030_exemption_binding` for this `(template_id, draft_revision_id)`. **Require exact set match** under the predicate `for_every fᵢ ∈ F: exists eⱼ ∈ E where eⱼ.analyzer_finding_fingerprint = fᵢ.finding_fingerprint AND exemption(eⱼ) is valid per binding-rules in step 4`. Any unmatched finding → reject with `forms.publish.i030_violation` Category B audit naming the specific finding `(category, jsonpath, finding_fingerprint)`. Any binding pointing at an analyzer-finding fingerprint NOT in `F` (an unused/stale exemption) → reject with `forms.publish.i030_exemption_stale` Category B audit (this prevents an exemption from being authored against a fabricated finding fingerprint, or an exemption from outliving the finding it was meant to cover).
4. False-positive carve-outs use a **one-to-many binding model**: a template draft may have N analyzer findings, each requiring its own paired exemption. Implementation: separate table `forms_template_i030_exemption_binding(binding_id PK, template_id, draft_revision_id, exemption_id FK → forms_i030_exemption, analyzer_finding_fingerprint)` with `UNIQUE(template_id, draft_revision_id, analyzer_finding_fingerprint)` — each finding has at most one bound exemption per draft revision. **Exemptions are narrow, non-reusable approval artifacts** scoped to a single analyzer finding:
