...
3. Publish path requires **one matching approval row per edit-log entry**:
   ...
   - **No superset matching.** Approval is bound exactly to one edit's diff artifact — a single approval cannot satisfy multiple edits, and a "broad" approval cannot rubber-stamp paths the approver didn't review (the row's path-set must equal the edit-log's path-set, enforced at INSERT).
