...
**Severity:** high
**Status:** **OPEN — v0.5 DRAFT** (Codex R4 HIGH ×1 closed: terminal-state audit/domain event enumeration; pre-ratification gate continues)
...
- **AUDIT_EVENTS v5.3 — promote placeholder set to canonical:** enumerate `refill.{requested, eligible, ineligible, signals_evaluated, clinician_approved, clinician_declined, protocol_approved, protocol_declined, fulfilling_started, released, dispatched, delivered, picked_up, completed, cancelled, **expired**, safety_hold_triggered, bridge_supply_dispensed, execution_rejected}` (19 net-new — `expired` added per Codex R4 HIGH closure 2026-05-14); `dispensing.{queued, claimed, released, exception_recorded, held, escalated, resolved}` (7 net-new); `shipment.{dispatched, in_transit_update, delivered, delivery_failed, pickup_available, picked_up, pickup_expired, **cancelled_before_dispatch**}` (8 net-new — `cancelled_before_dispatch` added per Codex R4 HIGH closure 2026-05-14). Bumps AUDIT_EVENTS Contracts Pack **v5.3 → v5.4** (smallest semver step appropriate to additive-only Category A enumeration; precedent = P-011 v5.2 → v5.3 amendment). **Total: 34 net-new** (was 32 in v0.3; +1 refill.expired +1 shipment.cancelled_before_dispatch per R4 closure).
- **DOMAIN_EVENTS v5.2 — additive enum extension:** add `refill.{approved, dispatched, completed, cancelled, **expired**, interaction_safety_hold_triggered}` (6 net-new tenant-scoped events; partition_key `tenant_id:refill_id`; `expired` added per Codex R4 HIGH closure 2026-05-14); `dispensing.{released, exception_escalated}` (2 net-new; partition_key `tenant_id:dispensing_id`); `shipment.{delivered, delivery_failed, **pickup_expired**, **cancelled_before_dispatch**}` (4 net-new; partition_key `tenant_id:shipment_id`; `pickup_expired` + `cancelled_before_dispatch` added per Codex R4 HIGH closure 2026-05-14). No version bump per P-011 precedent (DOMAIN_EVENTS additive enum extension stays in-place at v5.2). **Total: 12 net-new** (was 9 in v0.3; +1 refill.expired +1 shipment.pickup_expired +1 shipment.cancelled_before_dispatch per R4 closure).
...
Content-change promotion bumping Registry v2.11 → v2.12. Coverage counts updated: entities 42 → 45 (added Refill #19, Dispensing #20, Shipment #21); state machines 19 → 19 (no new SMs — §2 + §5 already canonical); Contracts Pack rows updated (AUDIT_EVENTS v5.3 → v5.4 with 34 net-new Category A action IDs; DOMAIN_EVENTS in-place at v5.2 with 12 net-new event types); CDM row updated to v1.4 with §4.17 + §4.18 + §4.19 + amended `audit_i012_workload_evidence_required` CHECK noted. (Counts updated v0.3 → v0.4 per Codex R4 HIGH closure: +1 refill.expired audit/domain; +1 shipment.cancelled_before_dispatch audit/domain; +1 shipment.pickup_expired domain — previously absent from the canonical promotion lists despite being introduced as terminal states.)
...
| AUDIT_EVENTS v5.3 → v5.4 promotion: enumerate 34 net-new Category A action IDs (19 refill + 7 dispensing + 8 shipment) replacing the placeholder set                                           | Privacy/Compliance + Engineering Lead         | high     |
| DOMAIN_EVENTS v5.2 in-place amendment: 12 net-new domain event types (6 refill + 2 dispensing + 4 shipment)                                                                                    | Engineering Lead                              | medium   |
...
- **Next:** v0.6 after Codex R5 review; iterate to convergence per the SI-001 trajectory pattern (R1 → R10 was SI-001's path).
