...
**Severity:** high
**Status:** **OPEN — v0.6 DRAFT** (Codex R5 HIGH ×2 closed: pickup-path two-step canonicalization, Dispensing.CANCELLED enum + event entries; pre-ratification gate continues)
...
- **AUDIT_EVENTS v5.3 — promote placeholder set to canonical:** enumerate `refill.{requested, eligible, ineligible, signals_evaluated, clinician_approved, clinician_declined, protocol_approved, protocol_declined, fulfilling_started, released, dispatched, delivered, picked_up, completed, cancelled, expired, safety_hold_triggered, bridge_supply_dispensed, execution_rejected}` (19 net-new — `expired` added per Codex R4 HIGH closure); `dispensing.{queued, claimed, released, exception_recorded, held, escalated, resolved, **cancelled**}` (8 net-new — `cancelled` added per Codex R5 HIGH closure 2026-05-14); `shipment.{dispatched, in_transit_update, delivered, delivery_failed, pickup_available, picked_up, pickup_expired, cancelled_before_dispatch}` (8 net-new — `cancelled_before_dispatch` added per Codex R4 HIGH closure). Bumps AUDIT_EVENTS Contracts Pack **v5.3 → v5.4**. **Total: 35 net-new** (was 34 in v0.5; +1 dispensing.cancelled per R5 closure).
- **DOMAIN_EVENTS v5.2 — additive enum extension:** add `refill.{approved, dispatched, completed, cancelled, expired, interaction_safety_hold_triggered}` (6 net-new tenant-scoped events; partition_key `tenant_id:refill_id`); `dispensing.{released, exception_escalated, **cancelled**}` (3 net-new — `cancelled` added per Codex R5 HIGH closure 2026-05-14; partition_key `tenant_id:dispensing_id`); `shipment.{delivered, delivery_failed, pickup_expired, cancelled_before_dispatch}` (4 net-new; partition_key `tenant_id:shipment_id`). No version bump per P-011 precedent. **Total: 13 net-new** (was 12 in v0.5; +1 dispensing.cancelled per R5 closure).
...
Content-change promotion bumping Registry v2.11 → v2.12. Coverage counts updated: entities 42 → 45 (added Refill #19, Dispensing #20, Shipment #21); state machines 19 → 19 (no new SMs — §2 + §5 already canonical); Contracts Pack rows updated (AUDIT_EVENTS v5.3 → v5.4 with 35 net-new Category A action IDs; DOMAIN_EVENTS in-place at v5.2 with 13 net-new event types); CDM row updated to v1.4 with §4.17 + §4.18 + §4.19 + amended `audit_i012_workload_evidence_required` CHECK noted. (Counts updated through v0.6: v0.3 → v0.4 +1 refill.expired audit/domain +1 shipment.cancelled_before_dispatch audit/domain +1 shipment.pickup_expired domain; v0.5 → v0.6 +1 dispensing.cancelled audit + +1 dispensing.cancelled domain per Codex R5 closure.)
...
| AUDIT_EVENTS v5.3 → v5.4 promotion: enumerate 35 net-new Category A action IDs (19 refill + 8 dispensing + 8 shipment) replacing the placeholder set                                           | Privacy/Compliance + Engineering Lead         | high     |
| DOMAIN_EVENTS v5.2 in-place amendment: 13 net-new domain event types (6 refill + 3 dispensing + 4 shipment)                                                                                    | Engineering Lead                              | medium   |
...
- **Next:** v0.7 after Codex R6 review; iterate to convergence per the SI-001 trajectory pattern (R1 → R10 was SI-001's path).
