...
- 🟡 Input screener on all **server-visible-plaintext** patient-facing free-text endpoints — regex + local NER PII detection (NEVER external LLM). **Backend portion COMPLETE** (Sprint 1.1a regex core, Sprint 1.1b wink-nlp NER, Sprint 1.1c wiring into `POST /v0/ai/chat`, the only server-visible plaintext route). **Evidence:** merges `0445186`, `3db62b7`, + Sprint 1.1c PR; Codex APPROVE on each.
- ⬜ **Client-side input screener for the async-consult family** — `POST /v1/async-consults/:id/{intake,decision,follow-up-messages}` receive pre-encrypted KMS envelopes per I-026; the server never sees plaintext, so server-side screening is architecturally impossible. Screening must run client-side before encryption in `telecheck-patient-app` (intake + follow-up composer) and `telecheck-clinician-console` (decision-rationale composer). **This is Track 4 work.** **Evidence required:** client-side screener implementation PR in each app + adversarial test + deployment verification. **Blocks Day-0 unless the ratifier explicitly accepts the interim risk** (participant training as the sole control for those three routes).
