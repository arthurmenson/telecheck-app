...
> **Implementation status (Sprint 1.3 phase B, 2026-09-08):** `scripts/pilot-1-baseline-seed.sql` (kept under `scripts/`, not `migrations/` — the test bootstrap and migration tooling apply every `.sql` under `migrations/`) seeds both operating tenants' baseline staff accounts plus one baseline test-fixture patient per tenant, every row with `cohort_classification = 'baseline'` named in the INSERT; idempotent via `ON CONFLICT DO NOTHING`; refuses to run if either tenant (created by migration 001, preserved by purge) is missing; a trailing `DO` block aborts if any seed row is not an active baseline row. `scripts/pilot-1-marker-remediation.sh` implements the remediation contract: one `unclassified` account per invocation, classified and attested (`pilot_1.cohort_classification`) in one transaction, refusing already-classified rows (no reclassify mode). `scripts/seed-staging-accounts.sql` now names the classification too. CI: `scripts/pilot-1-remediation.test.mjs` (stubbed psql control flow + static seed check) and `tests/integration/pilot-1-cohort-remediation.test.ts` (real Postgres). The `createAccount` classification argument is pending the ratifier's decision recorded in the spec repo's `Decision-Request-Pilot-1-Account-Classification-Provisioning-2026-09-08.md`.
...
> **Implementation status (Sprint 1.3 phase B part 2, 2026-09-08):** `scripts/pilot-1-env-purge.sh` implements both modes. Preconditions are read-only via `scripts/lib/incident-manifest.mjs` (manifest existence / SUCCESS / ≤30 min / id match / ≥1 structurally verified `<id>-*.age` artifact / `consumed:false` / lock match; routine-reset blocked by the lock or any unconsumed manifest); the cohort gate (`verify-pilot-1-baseline.sh`) must be green; incident mode is single-use via the `env.purge.executed` attestation. The transaction is: advisory lock → attestation rows FIRST (one per tenant — the resource tenant — with actor_tenant_id = the operator's home tenant, all sharing an operation id, PLATFORM partition, payload listing every tenant, the plan digest and the classification version) → the plan rendered by `scripts/lib/purge-plan.mjs` from the checked-in `scripts/pilot-1-purge-classification.json` (one `TRUNCATE … RESTRICT` naming every `allowlist` table, then each `scoped-delete` with its recorded predicate, then post-checks that abort if anything survived) → COMMIT; re-seed and the runtime steps follow. The script never writes under `incident-logs/`. CI: `scripts/pilot-1-env-purge.test.mjs` (DB-free) and `tests/integration/pilot-1-env-purge.test.ts` (schema-drift, FK-edge, seeded-canary purge, unclassified refusal, attestation-transaction rollback at three points, incident mode + single use + directory byte-for-byte). The care-intake immutability triggers (migration 095) are disabled for USER triggers only around the TRUNCATE and re-enabled + post-checked in the same transaction; `account_pin_credentials` and `auth_devices` are scoped-delete (participant-bound rows only, before the accounts delete). A psql failure without a refusal is reconciled against the durable attestation by operation id (exit 3 verified rollback / exit 5 outcome unknown, app left stopped); `--finish-runtime --operation-id <uuid>` completes re-seed + runtime steps after exit 4 / 5 only for a COMMITTED attestation whose incident state still matches. A kernel lifecycle lock (`flock` on a file that is never unlinked and must lie outside the incident tree) serialises every invocation from before the app is stopped until the runtime steps end; the materialized view over the interaction tables is refreshed inside the transaction and post-checked empty; the two replay caches (`idempotency_keys`, `identity_idempotency_keys`) are scoped-deleted so retained accounts keep their replay protection; Redis FLUSHALL must be acknowledged with OK; the lifecycle lock file and the temporary directory must resolve (symlinks followed) outside the incident tree; the lock file may not be a symbolic link; every manifest artifact entry's `ciphertextBytes` is mandatory and must equal the file's real size (an entry without it is an incomplete manifest and counts as FAILED); artifacts must be regular files whose real location lies inside the real incident directory (symlink artifacts refused); surviving replay-cache rows keep their key / endpoint / actor / request hash but their cached response body is replaced by a non-sensitive tombstone inside the purge transaction; runtime stage markers (`PILOT_1_RUNTIME_STATE_DIR/<operation-id>/…`, outside the incident tree) record each finished stage and the latest committed operation, so `--finish-runtime` resumes only the unfinished stages of the latest, not-yet-completed operation, fences live traffic (stops the app) before any remaining destructive stage, and retries a failed health check as a health check only; the incident lock's presence is established by lstat (a dangling or symlinked lock is a blocker); reconciliation of an ambiguous psql failure takes the same advisory lock (bounded by `lock_timeout`) before deciding; the health check requires exactly HTTP 200 from every configured URL and the URLs are mandatory when runtime steps run. The app logs to stdout, so the runtime step removes and recreates the app container. The CI suite runs against its own disposable database.
