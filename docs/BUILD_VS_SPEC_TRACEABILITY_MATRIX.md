...
| **SI-005** | consults / consult_events schema canonical ratification (currently shipped as engineering-authored placeholder per migrations 020 + 021) | OPEN — targets next-available P-NUM in the next ratification ceremony |
...
| **SI-008** | `ai_workflow_executions` schema gap (Mode 2 case-prep AI execution durability; AI Service module structure expansion gated) | OPEN — targets **P-022** alongside the other 7 pending SIs in the next ratification ceremony (filed 2026-05-15) |
| **SI-009** | `sync_sessions` schema gap (LiveKit-backed sync video consult session durability) | OPEN — targets **P-022** alongside the other 7 pending SIs (filed 2026-05-15) |
| **SI-010** | Session actor-context DB binding (R4 HIGH locked-down design; R5 HIGH supersedes R2 HIGH-2 per-request temp-table mandate) | OPEN — targets **P-022** alongside the other 7 pending SIs (filed 2026-05-15) |
| **SI-011** | Forms publish governance gates (clinical-lead + privacy-lead + RBAC-policy attestation sequencing for forms template publish) | OPEN — targets per-sub-SI ledger shape: P-021 umbrella + P-022..P-025 per sub-SI a/b/c/d (per PR #172 R1 M2 keystone; filed 2026-05-15) |
| **SI-012** | Medication Interaction CDM expansion (entity surface for Med Interaction Engine slice; gated until expansion ratifies) | OPEN — targets **P-022** alongside the other 7 pending SIs (filed 2026-05-16) |
| **SI-013** | CCR crisis-helpline keys (country-localized crisis-resource surface for Mode 1 chat per I-019 platform-floor) | OPEN — targets **P-022** alongside the other 7 pending SIs (filed 2026-05-16) |
| **SI-014** | Crisis-detection clinical NLP classifier (planned ADR-030 successor to today's regex-based crisis-detect; CRITICAL clinical-safety judgment requires ratifier sign-off) | OPEN — targets **P-022** alongside the other 7 pending SIs (filed 2026-05-16); ADR-030 author-class STOP-condition flagged |

**OPEN count: 12 SIs.** Spec corpus latest entry verified at `Telecheck_Promotion_Ledger.md` is **P-011** (SI-001 MedicationRequest canonical schema content-change promotion; 2026-05-11; spec corpus commit `879cd57`). P-012 was deferred. P-013 is claimed by SI-007. P-014 is the next slot, targeting SI-002. The other 9 OPEN SIs are queued for the next ratification ceremony (P-022+).
