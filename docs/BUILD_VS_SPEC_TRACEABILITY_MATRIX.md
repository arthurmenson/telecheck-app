...
**Scaffolds (NOT closes); closure path BUILT in-sprint, EXECUTION pending env-availability:**
- ORT row **OR-218** ("Performance and load test plan"; `Telecheck_Operational_Readiness_Todo_v1_5.md:129`) — **OPEN; closure path BUILT (Sprint 13)**. Sprint 7 / TLC-018 scaffolded `tests/perf/` infra. Sprint 11 / TLC-023a/b landed thresholds + `perf.yml` workflow. Sprint 12 / TLC-024 added `validateTransition` bench scenarios + TLC-023c handoff doc. Sprint 13 / TLC-026 landed in-sprint enforceable code: manifest-check helper + self-test mode (`tests/perf/check-thresholds.ts`) + machine-enforced baseline-refresh metadata guard (`.github/workflows/baseline-refresh-guard.yml` with full-line anchored regex + GH API validation + triple-dot merge-base diff + always-run + early-exit). All 3 ORT closure conditions per `tests/perf/README.md:5-11` are now either satisfied OR have machine-enforced closure paths. **Execution pending Evans-side `gh api` PUT** (per `docs/TLC-023c-BRANCH-PROTECTION-WIRE-UP.md` §1) **+ 3-5 stable `perf.yml` main runs** (Evans Option A signal). Sprint N+ flips OR-218 from "OPEN; closure path BUILT" to "FULLY CLOSED" post-Evans-execution.
