 ...
 *
 * **Failure discipline (Codex Pass-2 mandate):**
 ...
 *   - `RESET ROLE` is NOT issued at the end of the callback. `SET LOCAL`
 *     scopes the change to the transaction; commit/rollback automatically
 *     resets. Issuing `RESET ROLE` explicitly inside a savepoint-using
 *     `withTransaction` re-entry could leak the reset across savepoints
 *     in unexpected ways; rely on the tx-end auto-reset instead.
 *
...
export async function withDbRole<T>(
  ...
  // special characters, no uppercase, no quoting required. Direct
  // interpolation is safe BECAUSE the allowlist is the security
  // barrier (not the SQL escape).
  //
