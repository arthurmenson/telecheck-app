...
## Status: Sprint 1 (PR 9 of N handler series — this commit, merged with PR 8 from main) — **ALL 7 WRITE HANDLERS MOUNTED (PR 8: evaluations + signals + activate; PR 9: supersede + override + resolve + expire) + CAT A AUDIT EMISSION PATTERN ESTABLISHED**

Spec layer is **complete + ratified**. **DB layer is COMPLETE through migration 050** (PRs 1-5 merged; 21 Codex adversarial-review rounds total across the series). Handler series: PR 7 (GET /signals/:id) merged; PR 8 (POST /evaluations + POST /signals + POST /signals/:id/activate — first 3 write handlers establishing the Cat A audit emission pattern) merged to main and folded into this branch via the PR 8/PR 9 merge; **PR 9 (this commit) ships the 4 remaining write handlers** under the canonical `withTransaction → withTenantContext → withActorContext → withDbRole` composition: `supersede-signal` (OPERATIONAL — calls `record_signal_supersession` from migration 050 §3) + `override-signal` + `resolve-signal` + `expire-signal` (all 3 FAIL-CLOSED at the wrapper layer per Codex R1 closure 2026-05-23 — wrappers RAISE SQLSTATE `0A000` pending evidence-source migrations from Async Consult / per-basis cadence config / SI-024.1 JWT-binding). The handler scaffolds map `0A000` → tenant-blind 503 per I-025 and emit Cat A audit on rejection per I-003 bare-suppression-forbidden (the rejected attempt belongs in the audit chain).
