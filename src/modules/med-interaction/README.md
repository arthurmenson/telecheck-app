...
## Status: Sprint 2 (PR 7 — this commit) — **first Fastify handler: signal current-state read**

Spec layer is **complete + ratified**. **DB layer is COMPLETE through migration 050** (PRs 1-6 merged; 21 Codex adversarial-review rounds across the DB-layer series) and the **app-role acquisition foundation (migration 051 + `withDbRole`) is merged** (4-round Codex convergence; cockpit Addendum 81), which unblocks the handler series.

This PR (7) lands the **first Fastify handler** — the lowest-risk read endpoint per cockpit Addendum 81:

- **`GET /v0/med-interaction/signals/:id`** — current-state projection via the SECDEF access function `get_interaction_signal_current_state` from migration 048 (SI-019 Sub-decision 9 HOT-PATH DISPLAY read path). First handler-layer consumer of `withDbRole` (elevates into `medication_interaction_signal_viewer`). **Pure read: NO Cat A audit emission** (SI-019 §6 audit catalog has no read event). LAYER B at the route layer gates to `signal_viewer`-entitled roles (clinician + admin in the current JWT enum; pharmacist + ai_clinical_assistant land with their slices). Tenant-blind 404 per I-025 (cross-tenant or absent or malformed id all return the identical not-found envelope).

The `/ready` probe stays **503** until the rest of the handler surface (write/lifecycle endpoints, PR 8+) and the deferred live-DB read tests close.
...
| 7   | n/a       | **First Fastify handler (this commit)** — `GET /v0/med-interaction/signals/:id` current-state read via the SECDEF access function (048) + `withDbRole` elevation into `medication_interaction_signal_viewer`. LAYER B route gate (clinician + admin). Tenant-blind 404 (I-025). NO Cat A audit (read). Auth-gating + input-validation tests run in CI; the live-DB 200/404/cross-tenant read tests are `it.todo` pending the SECDEF-aware test-harness extension (cockpit Addendum 81 out-of-scope item #1). | (Codex pending)                                                                                             |
| 8+  | n/a       | **Remaining Fastify handlers** — 7 write/lifecycle endpoints per SI-019 §5 + CDM §6 OpenAPI v0.3 (evaluation init, signal emission, activate, override, supersede, resolve, expire). Cat A audit emission (6 events: 4 Cat A + 2 Cat B). Integration tests for tenant isolation + I-002 ordering invariant.                                                                                                                                                                                                  |
