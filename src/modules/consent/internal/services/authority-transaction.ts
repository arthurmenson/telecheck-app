 ...
 * in-flight request with it. (Codex round 3 on PR #303.) So this module
...
function signalRecordingClientDiscarded(): void {
  ...
  try {
    logger.error(
      { event: 'consent.recording_connection.discarded' },
      'Consent recording connection discarded: cleanup did not complete within its bound',
    );
  } catch {
    // Never let a logger failure replace the consent response.
  }
}
 ...
 * withActorContext(work)))` followed by `SET CONSTRAINTS <evidence>
 * IMMEDIATE` — is the defect class fixed for the crisis path in PR #302 and
 * the care-intake path in PR #303. `withTenantContext` DELETES the
 * per-backend tenant binding in its cleanup, before the outer COMMIT;
 * `consent_care_live_actor()` needs `kms_current_actor_context()`, which
 * needs `current_tenant_id()`, so at COMMIT nothing could re-validate
 * authority — and forcing the triggers IMMEDIATE consumed their events, so
 * the real COMMIT ran with no authority check and an actor nonce expiring
 * in that window was committed under expired authority
 * (`kms_current_actor_context()` compares against `clock_timestamp()`).
 *
 * Now this module owns the client: it sets the tenant binding, runs
 * BEGIN…COMMIT with both bindings live, and lets the deferred
...
export function consentAuthorityTransaction(authority: ConsentAuthority): typeof withTransaction {
  return async <T>(work: (tx: DbTransaction) => Promise<T>): Promise<T> => {
    ...
    const deadline = new Promise<never>((_, reject) => {
      armDeadline = () => {
        deadlineTimer = setTimeout(() => {
          reject(Object.assign(new Error('consent_unavailable'), { code: 'COMMIT_DEADLINE' }));
        }, COMMIT_DEADLINE_MS);
      };
    });
    ...
    const run = (async () => {
      ...
      try {
        ...
        const result = await withActorContext(client, authority.nonce, async () => {
          ...
          await authority.assertLive(client);
          let value: T;
          try {
    ...
    const unconfirmed = () => {
      ...
      client.release?.(true);
      return Object.assign(new Error('consent_unavailable'), { code: 'PT503' });
    };
