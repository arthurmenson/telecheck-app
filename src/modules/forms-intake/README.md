...
2. **`audit.ts`** — AUDIT*EVENTS v5.2 doesn't enumerate canonical action IDs for `forms_template*_`, `forms*deployment*_`, `forms*submission*_`, `forms*variant*_`, or `forms*resume_state*\*`lifecycle. All 12 unratified emissions across the module route through the`formsAuditPlaceholder()`helper — a typed-cast helper with a closed`FormsAuditActionPlaceholder`union containing template create/publish (2), deployment create/retire (2), submission start/paused/resumed/complete (4), and variant create/deployed/promote/retire (4). The single`as AuditAction`cast is contained inside the helper so`git grep "formsAuditPlaceholder("`inventories every unratified emission across the module. (Pre-migration 2026-05-04 a second pattern was used —`config_change_validated`action +`detail.intent` discriminator; the legacy-emitter-migration batch consolidated everything to the helper.) Engineering Lead should ratify all 12 canonical IDs in a future AUDIT_EVENTS amendment so the helper can be deleted entirely (3-step grep migration documented at the helper definition).
