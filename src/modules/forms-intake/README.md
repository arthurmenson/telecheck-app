...
2. **`audit.ts`** — AUDIT*EVENTS v5.2 doesn't enumerate canonical action IDs for `forms_template*_`, `forms*deployment*_`, `forms*submission*_`, `forms*variant*_`, or `forms*resume_state*\*`lifecycle. Most emitters use`<action> as AuditAction`casts (cleanest pattern); a few legacy ones (resume save/restore, variant deployed) still use the`config_change_validated`placeholder +`detail.intent`. Engineering Lead should ratify canonical IDs in a future AUDIT_EVENTS amendment so all emitters can normalize.
