...
- Initiates a crisis event by calling the SECDEF wrapper `record_crisis_initiation()` from migration 036 (granted EXECUTE to `crisis_initiator` role) and emitting the Cat A `crisis.detected` audit in the SAME atomic transaction (FLOOR-020 fail-closed; if either fails the whole tx rolls back so no orphan crisis_event row exists without its audit record).
- Composition: `requireTenantContext → requireClinicianActorContext → body validation → resolveActorTenantIdForAudit → withIdempotentExecution (which opens tx + binds tenant context for idempotency_keys RLS) → withTenantContext (rls.ts private binding for parity with PR 1) → (withActorContext when req.actorNonce bound) → withDbRole('crisis_initiator', ...) → SELECT record_crisis_initiation(...) → emitCrisisDetectedAudit(tx)`.
...
- Layer B authorization: closest-available `requireClinicianActorContext` role-gate (TODO: when JWT-role → DB-slice-role membership mapping lands, expand to `crisis_initiator` membership check covering clinician + on-call clinician + ai_mode1_service per SI-022 §7; the audit emitter's `actor_type` derivation expands at the same point to distinguish clinician vs ai_workload).
...
- Unit tests cover: §1 happy path composition, §2 tenant guard precedes wrap, §3 clinician role-gate precedes wrap, §4 13-case body validation matrix, §5 audit emitted in same tx after withDbRole returns + audit-emit failure propagates (I-003), §6 42501 → 403 for both SET LOCAL ROLE failure and wrapper LAYER C failure + non-42501 PG errors propagate unchanged, §7 actorNonce undefined skips withActorContext, §8 SQLSTATE 23505 → tenant-blind 409 with no tenant_id / server_signal_id leak.
