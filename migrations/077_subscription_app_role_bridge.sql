-- =============================================================================
-- File:    migrations/077_subscription_app_role_bridge.sql
-- Purpose: Bridge the 4 Subscription slice roles (migration 075) into the
...
--   062/063 (actor-context helpers), 075 (the 4 roles) applied.
...
--   SET LOCAL ROLE.
-- Rollback: migrations/rollback/077_rollback.sql
-- =============================================================================
...
BEGIN
    IF to_regrole('telecheck_app_role') IS NULL THEN
        RAISE EXCEPTION 'migration-077-precondition-failed: '
            'telecheck_app_role does not exist; apply the migration that '
            'creates the application login role (and 051) before 077.';
    END IF;
    ...
    IF array_length(v_missing_roles, 1) > 0 THEN
        RAISE EXCEPTION 'migration-077-precondition-failed: % of 4 '
            'subscription roles do not exist: %. Apply migration '
            '075_subscription_rbac_roles.sql BEFORE 077, then re-run.',
    ...
    RAISE NOTICE 'migration-077-summary: % grants applied (PG version %), '
...
BEGIN
    FOREACH v_role IN ARRAY v_slice_roles LOOP
        ...
        RAISE NOTICE 'migration-077-grant: helper EXECUTE granted to slice role %', v_role;
    END LOOP;
END $$;
...
BEGIN
    ...
    IF v_inherits THEN
        RAISE EXCEPTION 'migration-077-verification: telecheck_app_role is '
            'INHERIT; the Option B foundation requires NOINHERIT (051 §1).';
    END IF;

    FOREACH v_role IN ARRAY v_slice_roles LOOP
        IF NOT pg_has_role('telecheck_app_role', v_role::regrole, 'MEMBER') THEN
            RAISE EXCEPTION 'migration-077-verification: telecheck_app_role '
        ...
        IF NOT (
            ...
            RAISE EXCEPTION 'migration-077-verification: % missing SI-010 helper EXECUTE', v_role;
        END IF;
    END LOOP;

    RAISE NOTICE 'migration-077-verify: clean — telecheck_app_role NOINHERIT '
