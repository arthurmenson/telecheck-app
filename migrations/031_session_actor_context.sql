...
--
-- The trust anchor. Per-request row keyed solely by the high-entropy
-- nonce (R2-CRITICAL closure 2026-05-15 — supersedes the prior
-- (pg_backend_pid, txid, nonce) PK design).
--
-- Why nonce-only keying (R2 redesign):
--   The original design keyed by (pg_backend_pid, txid, nonce) to
--   prevent cross-tx interference. But R2 correctly identified that
--   the binder must run on a SEPARATE connection (dedicated
--   bind_actor_context_role pool) from the app connection that later
--   reads via _current_actor_context_row(). Different connections =
--   different pg_backend_pid() and different txid_current() values,
--   so the read predicate would never find the bound row. The
--   binder writes one set of (pid, txid); the app reads with a
--   different set; the lookup always returns "unbound."
--
--   The fix: drop the pid/txid keying. The nonce IS the trust anchor.
--   It's a high-entropy random UUID generated per-request by
--   authContextPlugin and carried to the DB layer ONLY via
--     - The auth pool's bind_actor_context() invocation (writes the row)
--     - The app pool's `SET LOCAL app.request_nonce = '<uuid>'`
--       statement at request start (carried to procedures via GUC)
--   _current_actor_context_row() looks up the row by
--     nonce = current_setting('app.request_nonce') AND expires_at > NOW()
--   No connection-identity predicate is needed (and using one would
--   break the cross-connection design).
--
-- Threat model with nonce-only keying:
--   - Attacker SQL on the app connection can SET app.request_nonce to
--     any value, but the value must correspond to a REAL row inserted
--     by the binder. Without EXECUTE on bind_actor_context, no
--     fabrication is possible.
--   - Attacker would need to know an OTHER user's nonce value to
--     spoof. Nonces are UUIDv4 (122 bits of entropy) generated
--     server-side and never logged. Treat as request-bound shared
--     secrets with strict logging discipline (LOG_REDACT_PATHS in
--     pino covers request_nonce; spec-corpus + ops contracts MUST
--     classify nonce as "secret" alongside JWT tokens and KMS keys).
--   - TTL is 5 minutes; expired nonces are rejected at read time.
--
-- Schema:
--   nonce                             UUID PK — request-bound shared
--                                     secret; the trust anchor.
--                                     Generated by authContextPlugin
--                                     (UUIDv4) and carried via auth-
--                                     pool bind + app-pool SET LOCAL.
...
--
-- UPSERT semantics: nonce IS the PK (R2 closure 2026-05-15 — supersedes
-- the prior pid/txid+nonce composite PK design). Same nonce + identical
-- identity is idempotent (refreshes expires_at). Same nonce + DIFFERENT
-- identity raises — preventing silent identity-overwrite. Bind happens
-- on the auth-pool connection; reads happen on the app-pool connection
-- via the nonce-only lookup (no pid/txid affinity required).
-- -----------------------------------------------------------------------------
