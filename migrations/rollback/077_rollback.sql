-- =============================================================================
-- File:    migrations/rollback/077_rollback.sql
-- Purpose: Rollback migration 077_subscription_app_role_bridge.sql.
--
-- Revokes telecheck_app_role's membership in the 4 Subscription slice roles
-- and the SI-010 actor-context helper EXECUTE grants. Run BEFORE the 075
    ...
    LOOP
        IF to_regrole(v_role) IS NOT NULL
           AND pg_has_role('telecheck_app_role', v_role::regrole, 'MEMBER') THEN
            RAISE EXCEPTION 'rollback-077-verification: telecheck_app_role still member of %', v_role;
        END IF;
    END LOOP;
END $$;
